...
3. RFC 0027 (Ad Hoc Teamwork and Convention Discovery, revision 2), for Sessions that admit Participants on capability match rather than Convener signature, and for the Three-Tier Convention Discovery Handshake of RFC 0027 section 3.4 (Tier 1 Schelling reduction; Tier 2 Bayesian observational inference; Tier 3 minimax-regret robust selection). The Convener gated admission model of section 3.6 of the present RFC remains the default; capability match and open admission are the additive extensions defined in section 3.3 of RFC 0027. RFC 0027 Theorem A.3 and its endogenous-selection generalization Theorem A.3' guarantee Unilateral Participation Dominance: an OAP-speaking Participant in a Session accrues at-least-as-large expected utility as a non-speaking Participant for any peer composition. Sessions with `admission_mode = "open"` SHOULD additionally declare an `aht_fallback_policy` per RFC 0027 section 3.4a, since open admission inherently exposes the Session to non-protocol-speaking peers.
