...
| RFC 0027 (Ad Hoc Teamwork, revision 2) | A personal agent and a provisioned agent MAY form an ad hoc team per RFC 0027. The Capability Announcement of RFC 0027 section 3.2 MAY include BYOA Attestation claims (`evidence.byoa_attestation_ref`). The Three-Tier Convention Discovery Handshake of RFC 0027 section 3.4 applies to provisioned agents without modification: a provisioned agent is treated as a Protocol-Follower when it speaks the protocol, observable non-follower otherwise. The AHT Fallback Policy of section 3.4a is declared per provisioned agent in its Provisioning Manifest, permitting different fallback classes for personal versus provisioned identity. The Unilateral Participation Dominance result (RFC 0027 Theorem A.3) ensures that protocol-following is incentive-compatible across personal and provisioned identity boundaries. |
