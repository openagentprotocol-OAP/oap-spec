...
The Reputation Profile is computed by the Foundation Trust Anchor as the time decayed weighted average of all Performance Records, with explicit handling for:
...
The exact aggregation formula MUST be published by the Foundation under `oap.reputation.aggregation.v1` and MUST be reproducible by any Marketplace that wishes to verify Reputation independently.
...
2. Initiate a dispute through the Foundation Dispute Resolution service.
3. Request deletion of Records that are factually incorrect, with adjudication by the Foundation.
