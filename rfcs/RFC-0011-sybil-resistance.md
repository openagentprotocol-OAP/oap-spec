...
6. Douceur, J. R. (2002). The Sybil Attack. *International Workshop on Peer-to-Peer Systems.* The original formal characterization of the attack class that this RFC defends against; the impossibility result that motivates the resource-cost approach of the operational defenses of section 4.
7. Yu, H., Kaminsky, M., Gibbons, P. B., and Flaxman, A. (2008). SybilGuard: Defending Against Sybil Attacks via Social Networks. *IEEE/ACM Transactions on Networking* 16(3). The social-graph fast-mixing assumption that grounds the reputation-network defense of section 4.3 and the formal bound of RFC 0029 Theorem B.1.
8. Wang, G., Mohanlal, M., Wilson, C., Wang, X., Metzger, M., Zheng, H., and Zhao, B. Y. (2013). Social Turing Tests: Crowdsourcing Sybil Detection. *Network and Distributed System Security Symposium (NDSS).* The empirical validation that crowdsourced Sybil detection achieves higher accuracy than algorithmic detection alone; grounds the Multi-Party Review extension of the Sybil defense in `papers/safety-and-policy-stack.md` Appendix A.9.
9. Song, D., Shi, E., Fischer, I., and Shankar, U. (2012). Cloud Data Protection for the Masses. *IEEE Computer* 45(1). The threat model under which a defender must assume that an adversary may simultaneously control identity-issuance, computation, and storage; grounds the requirement of section 4.5 that Sybil-resistance evidence be anchored into independently operated Transparency Logs (RFC 0021) so that no single operator's compromise breaks the defense.
