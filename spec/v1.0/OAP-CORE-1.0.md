...
### 18.4 Information Flow Control (Taint Tracking)

In multi-agent environments, Context leakage occurs when a high-clearance Agent summarizes protected data into the prompt of a low-clearance Agent. To prevent this, OAP requires runtime Information Flow Control (IFC). Any string generated by an LLM based on Context $C$ inherits a security label $L(C)$ ("Taint"). When an Agent invokes a downstream Tool or Agent $T$, the Pre Action Confidentiality Gate MUST verify that the clearance of the destination dominates or equals the sensitivity of the data ($L(C) \sqsubseteq L(T)$) before the network request is initiated.

### 18.5 Privileged Mode
...
```json
{
  ...
  "zkp": {
    "system": "groth16",
    "public_inputs": ["did:plc:abc123", "did:web:weatherpro.example", "pol_3a7b..."],
    "proof": "base64url..."
  },
  "signatures": [
...
To comply with the Right to be Forgotten (e.g., GDPR Art. 17), Receipts MAY replace `input_hash` and `output_hash` with a Zero-Knowledge State Proof (zk-SNARK) under the `zkp` property. A valid `zkp` proves that the Policy Engine evaluated the inputs and returned TRUE, without exposing the plaintext data to hashing. Receipts containing a valid `zkp` satisfy the tamper-evidence requirement without anchoring PII in the Transparency Log.
